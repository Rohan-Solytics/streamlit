...
from artifacts import NII_1Y_data

def create_bar_chart(selected_scenarios):
    filtered_data = NII_1Y_data[NII_1Y_data["Scenario"].isin(selected_scenarios)]
...
def app():
    # Select scenarios
    all_scenarios = NII_1Y_data["Scenario"].unique()

    with st.expander("Select Scenarios"):
        # Toggle state for Select/Deselect All button
        toggle_state = st.session_state.get("toggle_state_nii_1y", False)
        # Initialize selected_scenarios outside of the if condition
        selected_scenarios = st.session_state.get("selected_scenarios_nii_1y", all_scenarios)
        ...
        if select_deselect_all_button:
            ...
            st.session_state.toggle_state_nii_1y = not toggle_state
