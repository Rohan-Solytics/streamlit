...
import base64
from artifacts import NII_Report_data
...
def app():
    ...
    selected_classifications = st.selectbox("Select Classifications:", ["All Classification"] + list(NII_Report_data["CLASSIFICATION1"].unique()))
    if selected_classifications == "All Classification":
        classification_data = NII_Report_data
    else:
        classification_data = NII_Report_data[NII_Report_data["CLASSIFICATION1"] == selected_classifications]

    # Rename the index column to "Row_Id"
    classification_data.index.name = "Row_Id"
    ...
    for col_name in classification_data.columns:
    ...
    st.markdown('<div class="download-button"><a href="data:file/csv;base64,{}" download="table.csv">⬇️</a></div>'.format(
        base64.b64encode(classification_data.to_csv(index=True, encoding='utf-8').encode()).decode()),
